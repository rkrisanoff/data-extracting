...
class Model(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.multiheadattention_0 = nn.MultiheadAttention(batch_first=True, embed_dim=128, num_heads=4)
        self.layernorm_0 = nn.LayerNorm(normalized_shape=[128])
        self.multiheadattention_1 = nn.MultiheadAttention(batch_first=True, embed_dim=128, num_heads=4)
        self.layernorm_1 = nn.LayerNorm(normalized_shape=[128])
        self.linear_0 = nn.Linear(in_features=128, out_features=512)
        self.sigmoid_0 = nn.Sigmoid()
        self.linear_1 = nn.Linear(in_features=512, out_features=128)
        self.layernorm_2 = nn.LayerNorm(normalized_shape=[128])
        self.relu_0 = nn.ReLU()
        self.layernorm_3 = nn.LayerNorm(normalized_shape=[128])
