...
class Model(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.multiheadattention_0 = nn.MultiheadAttention(batch_first=True, embed_dim=256, num_heads=2)
        self.layernorm_0 = nn.LayerNorm(normalized_shape=[256])
        self.multiheadattention_1 = nn.MultiheadAttention(batch_first=True, embed_dim=256, num_heads=2)
        self.layernorm_1 = nn.LayerNorm(normalized_shape=[256])
        self.linear_0 = nn.Linear(in_features=256, out_features=512)
        self.sigmoid_0 = nn.Sigmoid()
        self.linear_1 = nn.Linear(in_features=512, out_features=256)
