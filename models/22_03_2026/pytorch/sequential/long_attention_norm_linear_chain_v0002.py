...
class Model(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.multiheadattention_0 = nn.MultiheadAttention(batch_first=True, embed_dim=64, num_heads=8)
        self.layernorm_0 = nn.LayerNorm(normalized_shape=[64])
        self.multiheadattention_1 = nn.MultiheadAttention(batch_first=True, embed_dim=64, num_heads=8)
        self.layernorm_1 = nn.LayerNorm(normalized_shape=[64])
        self.linear_0 = nn.Linear(in_features=64, out_features=128)
        self.sigmoid_0 = nn.Sigmoid()
        self.linear_1 = nn.Linear(in_features=128, out_features=64)
        self.layernorm_2 = nn.LayerNorm(normalized_shape=[64])
        self.relu_0 = nn.ReLU()
        self.layernorm_3 = nn.LayerNorm(normalized_shape=[64])
